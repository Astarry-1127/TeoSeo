...
class GetSite
{
    ...
    public static function getHeaderData($body_id = 'home', $archive = null, $custom_title = '', $custom_description = '', $custom_keywords = '')
    {
        ...
        $links = [
            [
                'rel'  => 'stylesheet',
                'href' => Get::Assets('assets/css/materialdesignicons.min.css')
            ],
            [
                'rel'  => 'stylesheet',
                'type' => 'text/css',
        ...
        if ($body_id === 'post' || $body_id === 'links') {
            $links[] = [
                'rel'  => 'stylesheet',
                'type' => 'text/css',
                'href' => Get::Assets('assets/css/markdown.css') . '?v=20260815' // v: 打破 EdgeOne/浏览器缓存(2026-08-10 GEO 折叠块样式)
            ];
            $links[] = [
                'rel'  => 'stylesheet',
                'href' => Get::Assets('assets/vendor/monokai-sublime.min.css')
            ];
        }
        ...
        if ($archive) {
            ...
            // 检查是否是文章/链接页面或归档页面且有自定义 SEO 信息
            // 归档页面（标签/分类/搜索等）Typecho 核心默认不输出 description，
            // 这里统一注入主题构建的描述，并移除 Typecho 输出的默认值
            $isCustomPage = (($body_id === 'post' || $body_id === 'links') && isset($archive->cid) && $archive->cid);
            $isArchivePage = ($body_id === 'archive');
            $hasCustomDescription = (($isCustomPage || $isArchivePage) && !empty($description) && $description !== self::description());
            $hasCustomKeywords = (($isCustomPage || $isArchivePage) && !empty($keywords) && $keywords !== self::keywords());
    ...
    public static function getFooterData($body_id = 'home', $archive = null)
    {
        ...
        $scripts = [
            ...
            [
                'type' => 'text/javascript',
                'src' => Get::Assets('assets/js/sweetalert2.all.min.js')
            ],
            [
                'type' => 'text/javascript',
        ...
        if ($body_id === 'post' || $body_id === 'links') {
            $scripts[] = [
                'type' => 'text/javascript',
                'src' => Get::Assets('assets/vendor/highlight.min.js')
            ];
            $scripts[] = [
                'type' => 'text/javascript',
