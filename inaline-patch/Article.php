...
class GetArticle
{
    ...
    private static function getPermalinkByCid($cid, $slug = '', $created = 0)
    {
        ...
        try {
            ...
            // 同时传入 cid 与 slug，避免 slug 形式固定链接下路由占位符未解析而输出 {slug}
            ...
            // 若仍存在未解析的占位符（例如路由缺少 cid），尝试仅用 cid 生成
    ...
    public static function getReaderData($archive = null)
    {
        ...
        // 再使用 Typecho 的 Markdown 解析器处理标准 Markdown(parseMarkdown 额外保护 <details> 折叠块)
        $content = MarkdownParser::parseMarkdown($content);
