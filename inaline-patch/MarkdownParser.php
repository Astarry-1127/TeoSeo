...
class MarkdownParser
{
    ...
    /**
     * 渲染 Markdown, 同时保护 <details> 折叠块
     *
     * parsedown(Utils\Markdown::convert) 不把 <details> 当块级 HTML,
     * 会在其内部插入 <br>/</p>, 导致 <summary> 不再是 <details> 的第一个
     * 子元素, 折叠功能与样式全部失效。渲染前用占位符保护, 渲染后恢复。
     *
    ...
    {
        $blocks = [];
        $content = preg_replace_callback('/<details>([\s\S]*?)<\/details>/s', function ($m) use (&$blocks) {
            $i = count($blocks);
            $blocks[$i] = $m[0];
            return '<inaline-details id="' . $i . '"></inaline-details>';
        }, $content);
        ...
        if ($blocks) {
            $content = preg_replace_callback(
                '/<p>\s*<inaline-details id="(\d+)">\s*<\/inaline-details>\s*<\/p>|<inaline-details id="(\d+)"><\/inaline-details>/',
